// AVL.hpp -- definition of AVL-Tree class,				//
...
#ifndef DICTIONARY_AVL_AVL_HPP_
#define DICTIONARY_AVL_AVL_HPP_
#include "AVL.h"
...
const D& AVL<K, D>::find(const K& key) const
{
	...
	TreeNode*& node = m_find(key, m_root);
...
template <typename K, typename D>
bool AVL<K, D>::contains(const K& key) const
{
...
template <typename K, typename D>
typename AVL<K, D>::TreeNode*& AVL<K, D>::m_find(
	const K& key, TreeNode*& cur) const
{
	...
	else if (key < cur->key) { return m_find(key, cur->left); }
	...
	else { return m_find(key, cur->right); }
}
...
void AVL<K, D>::insert(const K& key, const D& data)
{
	...
	m_find_and_insert(key, data, m_root);
}

template <typename K, typename D>
void AVL<K, D>::m_find_and_insert(const K& key, const D& data, TreeNode*& cur)
{
	...
	else if (key < cur->key)
	{
		...
		m_find_and_insert(key, data, cur->left);
	...
	else
	{
		...
		m_find_and_insert(key, data, cur->right);
		// On the way back up, ensure the balance of this node
		m_ensureBalance(cur);
		return;
	}
...
const D& AVL<K, D>::remove(const K& key)
{
	...
	// while it returns.
	const D& d = m_find_and_remove(key, m_root);

	return d;
}

template <typename K, typename D>
const D& AVL<K, D>::m_find_and_remove(const K& key, TreeNode*& cur)
{
	...
	else if (key < cur->key)
	{
		...
		const D& d = m_find_and_remove(key, cur->left);
	...
	else
	{
		...
		const D& d = m_find_and_remove(key, cur->right);
		// Ensure balance and update height of this ancestor
		// on the way back up the call stack:
		m_ensureBalance(cur);
		return d;
...
// m_remove will remove the node pointed to by the argument. Note that this
...
template <typename K, typename D>
const D& AVL<K, D>::m_remove(TreeNode*& node)
{
...
template <typename K, typename D>
typename AVL<K, D>::TreeNode*& AVL<K, D>::m_swap_nodes(
...
template <typename K, typename D>
void AVL<K, D>::m_updateHeight(TreeNode*& cur) 
{
	...
	cur->height = 1 + std::max(m_get_height(cur->left), m_get_height(cur->right));
}

template <typename K, typename D>
void AVL<K, D>::m_ensureBalance(TreeNode*& cur) 
{
	// Base case for safety: do nothing if cur is nullptr.
	if (!cur) return;

	// Calculate the balance factor for this node:
	const int initial_balance = m_get_balance_factor(cur);
	...
	if (initial_balance < -2 || initial_balance > 2) 
	{
		...
		std::cerr << "hl: " << m_get_height(cur->left) << " hr: " << m_get_height(cur->right) << std::endl;
	...
	{
		const int l_balance = m_get_balance_factor(cur->left);
	...
	{
		const int r_balance = m_get_balance_factor(cur->right);
	...
	const int final_balance = m_get_balance_factor(cur);
...
template <typename K, typename D>
void AVL<K, D>::m_rotateLeft(TreeNode*& cur) 
{
...
template <typename K, typename D>
void AVL<K, D>::m_rotateRight(TreeNode*& cur)
{
...
template <typename K, typename D>
void AVL<K, D>::m_rotateRightLeft(TreeNode*& cur)
{
	// Here, cur points to the original top-most node that roots the subtree
...
template <typename K, typename D>
void AVL<K, D>::m_rotateLeftRight(TreeNode*& cur) 
{
...
template <typename K, typename D>
const D& AVL<K, D>::m_iopRemove(TreeNode*& targetNode) 
{
	...
	const D& d = m_iopRemove(targetNode, targetNode->left, true);
	...
	if (targetNode->left) {
		m_ensureBalance(targetNode->left);
	}
	m_ensureBalance(targetNode);

	return d;
}

template <typename K, typename D>
const D& AVL<K, D>::m_iopRemove(TreeNode*& targetNode, TreeNode*& iopAncestor, bool isInitialCall) 
{
	...
	if (iopAncestor->right != nullptr) 
	{
		...
		const D& d = m_iopRemove(targetNode, iopAncestor->right, false);
	...
	else 
	{
		...
		TreeNode*& movedTarget = m_swap_nodes(targetNode, iopAncestor);
		...
		// that was removed.
		const D& d = m_remove(movedTarget);
		return d;
	}
...
#include "AVL_print.hpp"

#endif // !DICTIONARY_AVL_AVL_HPP_
